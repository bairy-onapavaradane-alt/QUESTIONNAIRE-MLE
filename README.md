...
// Recommandations complètes Mission Locale
const recommandations = (profil) => {
  if (profil === "fragile") {
    return [
      "PSAD / Obligation de formation",
      "TAPAJ",
      "BIP",
      "Aides financières (FDAJ / FUI)",
      "Accompagnement santé / psychologique",
      "Solutions logement (ALT / Maison Relais)"
    ];
  }
  ...
  if (profil === "intermediaire") {
    return [
      ...
      "Jeunes et logés",
      "BIP",
      "Ateliers emploi / formation",
      "Mobilité locale et externe"
    ];
  }

  return [
    "Accès direct emploi",
    "Alternance / formation qualifiante",
...
const calculProfil = (r) => {
  ...
  if (r.logement === "stable") score++;
  if (r.sante === "ok") score++;
  if (r.mobilite === "ok") score++;
  if (r.projet === "clair") score += 2;
  if (r.competences >= 7) score += 2;
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white p-6 rounded-2xl shadow w-full max-w-md">
        <h1 className="text-xl font-bold text-center mb-4">
          Diagnostic Jeune - Mission Locale
        </h1>
